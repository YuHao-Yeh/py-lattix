...
from src.lattix.utils import common as common
...
class TestDeepConvert:
    ...
    def test_primitives(self):
        ...
        assert common.deep_convert(1) == 1
        assert common.deep_convert("string") == "string"
        assert common.deep_convert(None) is None
    ...
    def test_adapter_usage(self):
        ...
        with patch.object(common, "get_adapter", return_value=mock_adapter):
            result = common.deep_convert(obj)
    ...
    def test_mapping_to_dict(self):
        ...
        data = {"a": 1, "b": {"c": 2}}
        result = common.deep_convert(data, dict)
    ...
    def test_mapping_to_list(self):
        ...
        # -> [["a", 1], ["b", 2]]
        data = {"a": 1, "b": 2}
        result = common.deep_convert(data, list)
        ...
        # -> ["a", 1] -> ["a", 1]
        data_single = {"a": 1}
        result_single = common.deep_convert(data_single, list)
    ...
    def test_mapping_to_tuple(self):
        ...
        data = {"a": 1}
        # edge case: (("a", 1),) -> ("a", 1)
        result = common.deep_convert(data, tuple)
        assert result == ("a", 1)
    ...
    def test_mapping_unwrap_fail_not_list_or_tuple(self):
        ...
        result = common.deep_convert(data, ftype=WeirdContainer)
    ...
    def test_mapping_unwrap_fail_wrong_length(self):
        ...
        result = common.deep_convert(data, ftype=TripleContainer)
    ...
    def test_iterable_conversion(self):
        ...
        data_mixed = [{"a": 1}]
        result_mixed = common.deep_convert(data_mixed, list)
    ...
    def test_fallback(self):
        ...
        with patch.object(common, "is_primitive", return_value=False):
            # Also ensure get_adapter returns None
            with patch.object(common, "get_adapter", return_value=None):
                assert common.deep_convert(obj) == obj
...
class TestSerialize:
    def test_primitives(self):
        assert common.serialize(1) == 1
        assert common.serialize("s") == "s"
        assert common.serialize(None) is None
    ...
    def test_adapter_priority(self):
        ...
        obj = {"a": 1}
        with patch.object(common, "get_adapter", return_value=lambda x, r: "adapted"):
            assert common.serialize(obj) == "adapted"

    def test_mapping(self):
        """Test mapping serialization including key stringification."""
        data = {1: "one", "two": 2}
        result = common.serialize(data)
    ...
    def test_fallback_str_repr(self):
        ...
        assert common.serialize(GoodStr()) == "good"

        # Case 2: Bad str, fallback to repr
        class BadStr:
            def __str__(self): raise ValueError("No string for you")
            def __repr__(self): return "fallback_repr"
        
        assert common.serialize(BadStr()) == "fallback_repr"
...
class TestFlattenUnflatten:
    def test_flatten(self):
        data = {"a": 1, "b": {"c": 2}, "d": {}}
        result = common.flatten(data, sep="/")
        assert result["a"] == 1
        assert result["b/c"] == 2
        assert result["d"] == {}
    ...
    def test_flatten_non_dict_in_stack(self):
        ...
        """
        assert common.flatten("not_a_dict") == {"": "not_a_dict"}

    def test_unflatten_success(self):
        data = {"a": 1, "b.c": 2}
        result = common.unflatten(data, sep=".")
        assert result == {"a": 1, "b": {"c": 2}}
...
class TestScanClassAttrs:
    def test_scan_attrs(self):
        ...
        attrs = common.scan_class_attrs(Child)
    ...
    def test_scan_excludes_object(self):
        ...
        common.scan_class_attrs.cache_clear()
        
        class Simple:
            x = 1
            
        attrs = common.scan_class_attrs(Simple)
        assert "x" in attrs
    
    def test_scan_cache(self):
        class A:
            pass
        common.scan_class_attrs(A)
        common.scan_class_attrs(A)


# ---------- Tests 5: Other Helpers ----------

class TestHelpers:
    def test_strip_ansi(self):
        text = "\x1b[31mRed\x1b[0m Text"
        assert common.strip_ansi(text) == "Red Text"
        text = "\x1b[31mHello\x1b[0m"
        assert common.strip_ansi(text) == "Hello"

    def test_is_primitive(self):
        assert common.is_primitive(1) is True
        assert common.is_primitive(object()) is False

    def test_is_scalar_primitive(self):
        assert common.is_scalar(1) is True
    ...
    def test_is_scalar_pandas(self):
        ...
        with patch(f"{top_mod}.utils.common.HAS_PANDAS", True), \
            patch(f"{top_mod}.utils.common.pd", fake_pd):
            
            df = fake_pd.DataFrame()
            assert common.is_scalar(df) is True
            assert common.is_scalar(object()) is False
    ...
    def test_is_scalar_numpy(self):
        ...
        with patch(f"{top_mod}.utils.common.HAS_NUMPY", True), \
            patch(f"{top_mod}.utils.common.np", fake_np):
            
            arr = fake_np.ndarray()
            assert common.is_scalar(arr) is True

    def test_is_scalar_false(self):
        with patch(f"{top_mod}.utils.common.HAS_PANDAS", False), \
            patch(f"{top_mod}.utils.common.HAS_NUMPY", False):
            assert common.is_scalar(object()) is False
